<?php
/*=======================================================================
 // File:        QR-UTILS.INC.PHP
 // Description: Utility function for bit and string manipulation
 // Created:     2006-08-20
 // Ver:         $Id: qr-utils.inc.php 1106 2009-02-22 20:16:35Z ljp $
 //
 // Copyright (c) 2008 Asial Co. Ltd. (Asial USA) All rights reserved.
 //========================================================================
 */
...
class Utils {
    ...
    // of consecutive bits
    //--------------------------------------------------------------------------------------------------------------------
    static function ByteArray2Bits($aData, &$aBits, $aWordSize = 8) {
        $resIdx = 0;
        $n = count($aData);
    ...
    // of consecutive bits
    //--------------------------------------------------------------------------------------------------------------------
    static function Word2Bits($aByte, &$aBits, $aWordSize = 8) {
        $resIdx = 0;
        $aBits = array();
        $maskbit = 1 << ($aWordSize-1);
        $mask = (1 << $aWordSize)-1;
...
function tilde_process($aStr) {
    ...
    while (($n = strpos($r, '~7', $offset)) !== false) {
        ...
        $seci = substr($r, $n + 2, 6);
        if (strlen($seci) < 6 || !is_numeric($seci)) return false;
    ...
    while (($n = strpos($r, '~d', $offset)) !== false) {
        ...
        $sv = substr($r, $n + 2, 3);
        if (strlen($sv) < 3 || !ctype_digit($sv)) return false;

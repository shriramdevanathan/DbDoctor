<?php
//=======================================================================
// File:        PDF417_ERROR.INC
// Description: Calculate the Reed-Solomon error corection
// Created:     2004-03-01
// Ver:         $Id: pdf417_error.inc.php 1211 2009-05-17 14:22:25Z ljp $
//
// Copyright (c) Asial Co. Ltd. (Asial USA) All rights reserved.
//========================================================================


// This class currently only handles the calculation of Reed-Solomon
// code words. It does not handle decoding and error correction using
// esisting data and code words.
class ReedSolomon {

    // Reed-Solomon Gnereator Polynomial Coefficients

    private $iPolCoeff = array(
    ...
    /*  Level 3 */
    array(274, 562, 232, 755, 599, 524, 801, 132, 295, 116, 442, 428, 295,  42, 176,  65 ),

    /*  Level 4 */
    array(361, 575, 922, 525, 176, 586, 640, 321, 536, 742, 677, 742, 687, 284, 193, 517,
    273, 494, 263, 147, 593, 800, 571, 320, 803, 133, 231, 390, 685, 330, 63, 410 ),

    /*  Level 5 */
    array(539, 422, 6, 93, 862, 771, 453, 106, 610, 287, 107, 505, 733, 877, 381, 612,
    ...
    /*  Level 6 */
    array(521, 310, 864, 547, 858, 580, 296, 379,  53, 779, 897, 444, 400, 925, 749, 415,
    ...
    /*  Level 7 */
    array(524, 894, 75, 766, 882, 857, 74, 204, 82, 586, 708, 250, 905, 786, 138, 720,
    ...
    /*  Level 8 */
    array(352, 77, 373, 504, 35, 599, 428, 207, 409, 574, 118, 498, 285, 380, 350, 492,
    ...
    // Checksums for the Reed-Solomon Tables above to guarantee file integrity
    private $iChkSum = array(array(701,63),array(40,888), array(632,49),
    array(141,603),array(818,93),array(413,413),
    array(80,232),array(399,117), array(151,444));

    function __construct() {
        // Check integrity of code and code tables
        $n = count($this->iPolCoeff);
        for($i=0; $i < $n; ++$i) {
            $coeff = $this->iPolCoeff[$i];
            $cw = $this->GetCodewords($coeff,0);
            /*
             echo "array( ";
             for( $j=0; $j < count($cw); ++$j ) {
             echo $cw[$j].",";
             }
             echo " ),\n";
             */
            if( $this->iChkSum[$i] != $cw ) {
                JpGraphError::RaiseL(25013); //'Coefficient tables corrupted');
            }
        }
    }

    function GetNumCodewords($aLevel=1) {
        return 1 << ($aLevel+1);
    }
    ...
    function GetCodewords($aData,$aLevel=1) {
        $n = count($aData);
        if( $n == 0 )
        JpGraphError::RaiseL(25014); //'No data to calculate codewords on.');
        $k = 1 << ($aLevel+1);
        $ck=array();
        $m = 929;
        for($i=0; $i < $k; ++$i ) $ck[$i] = 0;
        $t1 = $t2 = 0;
        for( $i=0; $i < $n; ++$i ) {
            $t1 = ($aData[$i]+$ck[$k-1]) % $m;
            for( $j=$k-1; $j > 0; --$j ) {

<?php
/*=======================================================================
// File: 	CRC-CCITT.PHP
// Description:	Classes to handle CRC-CCITT calculation
// Created: 	2006-08-18
// Ver:		$Id: crc-ccitt.inc.php 988 2008-03-25 02:50:13Z ljp $
//
// Copyright (c) 2006 Asial Co. Ltd. (Asial USA) All rights reserved.
//========================================================================
*/

/*-------------------------------------------------------------------------
 ** Class CRC_CCITT
 **
 ** Description: 
 ** Calculates CRC-CCITT checksum value using the specified polynom 
 **
 ** Poly			Hex		Comment
 ** ----------------------------------------------------------------
 ** x^16 + x^12 + x^5 + 1       0x1021		CRC-CCITT, X25
 ** x^16 + x^15 + x^2 + 1	0x8005		CRC16
 **
 ** with user selectable initial state. Depending on the application
 ** both initial state 0x0000 and 0xFFFF are commonly used. 
 **
 ** The class also includes facility to by7te revers a data buffer since
 ** some protocol specifications requires the data to be byte wise
 ** reversered before they are feed to the CRC calculation routine.
 **
 **-------------------------------------------------------------------------
 */
class CRC_CCITT {

    private $iPolynomial=0x1021;
    private $iInitialState=0x0000;
    private $iByteRevTable = array(
	);

	
    function CRC_CCITT($aPoly=0x1021,$aInitState=0x000) {
	$this->iPolynomial = $aPoly;
	$this->iInitialState = $aInitState;
    }

    function ByteReverse(&$aData) {
	$n = count($aData);
	for($i=0; $i < $n; ++$i ) {
	    $aData[$i] = $this->iByteRevTable[$aData[$i]];
	}
    }

    function Get($aData,$aConvert=true) {
	$crc16c = $this->iInitialState;  
	$n = count($aData);
	for ($i = 0; $i < $n; $i++) {
	    if( $aConvert ) 
		$word16 = ord($aData[$i]);
	    else
		$word16 = $aData[$i];

	    // The byte reversal is not really part of the CRC-CCITT but is
	    // specified in the Datamatrix specs as necessary
	    $word16 = $this->iByteRevTable[$word16];

	    $word16 <<= 8;
	    for($j = 0; $j < 8; $j++) {
		$xor_flag = (($crc16c ^ $word16) & 0x8000); 
		$crc16c <<= 1;
		if ($xor_flag) {
		    $crc16c ^= $this->iPolynomial;
		}
		$word16 <<= 1;
	    }
	    $crc16c &= 0x0000ffff;
	}
	return $crc16c;
    }

    function _UnitTest() {
	// Test1
	$test_data = array(0x03,0x00,0x41,0x42,0x31,0x32,0x2d,0x58);
	$test_data_crc = 0x9aae;

	//$this->ByteReverse($test_data);
	$crc_res = $this->Get($test_data,false);
	if( $crc_res == $test_data_crc ) {
	    printf("CRC-CCITT = 0x%X (CORRECT)\n",$crc_res);
	    $bits = array();
	    Word2Bits($crc_res,$bits,16);
	    print_r($bits);
	    return true;
	}
	else {
	    printf("CRC-CCITT = 0x%X WRONG !!! Should be \n",$crc_res,$test_data_crc);
	    return false;
	}
    }

}

//$utst = new CRC_CCITT();
//$utst->_UnitTest();


?>
